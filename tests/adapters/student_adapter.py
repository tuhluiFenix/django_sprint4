...
class StudentModelAdapter(ModelAdapter):
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        assert len(item_field_names) == len(item_fields), (
            f'В тестах к модели `{self.ItemModel.__name__}` для поиска полей, '
            'описанных в задании, используются типы этих полей, '
            'а в случае внешних ключей - ещё и типы связанных моделей. '
            f'Убедитесь, что в модели `{self.ItemModel.__name__}` нет полей, '
            'которые не описаны в задании, и для полей правильно заданы типы. '
            'В этом случае поля должны однозначно идентифицироваться '
            'по их типу и (в случае если поле - внешний ключ) '
            'типу связанных моделей.'
        )
